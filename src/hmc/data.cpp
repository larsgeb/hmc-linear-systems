...
namespace hmc {
    ...
    algebra_lib::matrix data::calculate_inverse_data_covariance(double std) {
        // Covariance is interpreted as absolute value of all datapoints.
        return (1.0 / (std * std)) * (algebra_lib::matrix(_numberData, _numberData).Unit());
    }

    algebra_lib::matrix data::calculate_inverse_data_covariance_percentual(double percentage) {
        // Covariance is interpreted as percentual value of measured value.
        algebra_lib::vector invStd = algebra_lib::ElementWiseDivision(1.0, (percentage / 100.0) * _observedData);

        return algebra_lib::VectorToDiagonal(algebra_lib::ElementWiseMultiplication(invStd, invStd));

    }

    double data::misfit(algebra_lib::vector &in_parameters) {
        return 0.5 * ((_G * in_parameters - _observedData).TransposeSelf()
                      * _inv_cov_d * (_G * in_parameters - _observedData)
        );
    }

    algebra_lib::vector data::gradient_misfit(algebra_lib::vector &parameters) {
